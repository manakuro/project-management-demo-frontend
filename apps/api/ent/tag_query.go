...
func (tq *TagQuery) QueryWorkspace() *WorkspaceQuery {
...
func (tq *TagQuery) QueryColor() *ColorQuery {
	query := &ColorQuery{config: tq.config}
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := tq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (tq *TagQuery) FirstID(ctx context.Context) (id ulid.ID, err error) {
	var ids []ulid.ID
...
func (tq *TagQuery) FirstIDX(ctx context.Context) ulid.ID {
...
func (tq *TagQuery) OnlyID(ctx context.Context) (id ulid.ID, err error) {
	var ids []ulid.ID
...
func (tq *TagQuery) OnlyIDX(ctx context.Context) ulid.ID {
...
func (tq *TagQuery) IDs(ctx context.Context) ([]ulid.ID, error) {
	var ids []ulid.ID
...
func (tq *TagQuery) IDsX(ctx context.Context) []ulid.ID {
	ids, err := tq.IDs(ctx)
	if err != nil {
		panic(err)
	}
...
func (tq *TagQuery) WithWorkspace(opts ...func(*WorkspaceQuery)) *TagQuery {
	query := &WorkspaceQuery{config: tq.config}
...
func (tq *TagQuery) WithColor(opts ...func(*ColorQuery)) *TagQuery {
	query := &ColorQuery{config: tq.config}
	for _, opt := range opts {
		opt(query)
	}
...
func (tq *TagQuery) sqlAll(ctx context.Context) ([]*Tag, error) {
	...
	if query := tq.withColor; query != nil {
		ids := make([]ulid.ID, 0, len(nodes))
		nodeids := make(map[ulid.ID][]*Tag)
		for i := range nodes {
			fk := nodes[i].ColorID
